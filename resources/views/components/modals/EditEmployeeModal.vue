<template>
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-gray-900 bg-opacity-50">
      <div class="bg-white rounded-lg shadow-lg w-11/12 max-w-3xl p-12">
        <div class="flex justify-between items-center mb-6">
          ...
          <button @click="closeEditEmployeeModal" class="text-gray-500 hover:text-gray-800">
        ...
        <form @submit.prevent="submitForm">
          ...
          <div class="flex flex-col w-full my-5">
            <button type="submit" class="bg-blue-500 text-white rounded-lg px-4 py-2 w-full">Update Employee</button>
          </div>
        </form>

        <div class="flex flex-col w-full">
          <button @click="closeEditEmployeeModal" class="text-gray-900 rounded-lg px-4 w-1/3 mx-auto">Cancel</button>
        </div>
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    data() {
      return {
        form: this.initializeForm(this.employee)
      };
    },
    methods: {
      ...
      async submitForm() {
        ...
        try {
          const response = await axios.put(`/api/employees/${this.employee.id}`, formData, {
            headers: {
