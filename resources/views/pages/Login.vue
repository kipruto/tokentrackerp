                  ...
                  <div class="relative px-4 sm:px-8">
                    <div class="max-w-lg mx-auto text-center">
                      ...
                      <form @submit.prevent="handleSubmit">
                        <input
                          v-model="email"
                          class="w-full mb-4 px-12 py-3 border border-gray-200 focus:ring-blue-300 focus:border-blue-300 rounded-md"
                          type="text"
                        ...
                        <button
                          id="loginButton"
                          class="w-full mt-4 bg-blue-800 hover:bg-blue-900 text-white font-bold font-heading py-3 rounded-md uppercase"
                          type="submit"
                        >
                          SUBMIT
                        </button>
  ...
  export default {
    ...
    const phpVersion = ref('loading...');
      ...
      return {
        ...
        backgroundImage// Adjust the path as needed
      };
    },
    mounted() {
    // Fetch PHP version from the backend
    axios.get('/api/php-version') // Make sure this URL is correct (prefix with baseURL if needed)
      .then(response => {
    ...
    async handleSubmit() {
      ...
      if (!this.validateEmail(this.email)) {
        ...
        try {
          ...
          this.setUser(response.data.user); // Store user in Vuex
          localStorage.setItem('user', JSON.stringify(response.data.user)); // Optionally store in local storage
